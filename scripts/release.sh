...
VERSION_FILE="$PROJECT_ROOT/apimeter/__init__.py"
...
release() {
    ...
    if publish_package "$test_pypi"; then
        ...
        if [[ "$test_pypi" == "true" ]]; then
            ...
            echo "pip install -i https://test.pypi.org/simple/ apimeter==$new_version"
        else
            echo -e "${BLUE}📦 安装命令:${NC}"
            echo "pip install apimeter==$new_version"
        fi
    else
        log_error "发布失败"
        exit 1
