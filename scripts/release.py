...
class ReleaseManager:
    ...
    def __init__(self, project_root: str = None):
        ...
        self.version_file = self.project_root / "apimeter" / "__init__.py"
    ...
    def create_and_push_tag(self, version: str, message: str = None):
        ...
        # 检查标签是否已存在
        returncode, _, _ = self.run_command(f"git tag -l {tag_name}", check=False)
        if returncode == 0:
    ...
    def release(self, new_version: str = None, test_pypi: bool = False, 
        ...
        if self.publish_package(test_pypi):
            ...
            if test_pypi:
                print("📦 测试安装命令:")
                print(f"pip install -i https://test.pypi.org/simple/ apimeter=={new_version}")
            else:
                print("📦 安装命令:")
                print(f"pip install apimeter=={new_version}")
        else:
            print("❌ 发布失败")
            sys.exit(1)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="ApiMeter 自动化发布脚本")
